  ...
  const handleSubmit = async (formData) => {
    ...
    try {
      ...
      const materiaPrimaBaseExistente = materiasPrimasBaseExistentes.find(mp => 
        mp.Nombre.toLowerCase() === formData.nombre.toLowerCase().trim()
      );

      if (materiaPrimaBaseExistente) {
        // Si existe, actualizar la cantidad
        const nuevaCantidad = materiaPrimaBaseExistente.Cantidad + formData.cantidad;
        ...
        setSuccess(`Cantidad actualizada exitosamente. Total: ${nuevaCantidad} ${formData.unidad}`);
        setTimeout(() => {
          setSuccess('');
          handleCloseModal();
          handleCreated();
